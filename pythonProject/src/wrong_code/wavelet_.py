...
class WaveletTransformTool:
    ...
    def cwt_windowed(self, window_len: int = 8192, overlap_frac: float = 2 / 3,
        ...
        for ch in range(n_ch):
            for i, start in enumerate(starts):
                ...
                # 暂时取消 COI：用全 1 掩膜
                mask = np.ones((n_freq, window_len), dtype=np.float64)

                coeffs_masked = coeffs * mask  # 实际等于 coeffs
                # 时间降采样……（不变）
                # 累加 acc_cfs…… acc_weight……
    ...
    def save_cwt_results(self, output_path: str) -> None:
        """
        保存 CWT 结果为 HDF5，格式兼容绘图工具。
        根数据集：
            coeffs      shape (n_freq, n_time, n_channels)
            frequencies shape (n_freq,)
            time_axis   shape (n_time,)
            coi         shape (n_time, n_channels)  （可选）
        """
        if not self.cwt_results:
            raise RuntimeError("没有 CWT 结果，请先调用 cwt_windowed()")

        # 组装三维数组 (n_freq, n_time, n_channels)
        coeffs_list = []
        for name in self.channel_names:
            coeffs_list.append(self.cwt_results[name]['coeffs'])
        cfs_all = np.stack(coeffs_list, axis=-1)  # (n_freq, n_time, n_channels)

        with h5py.File(output_path, 'w') as f:
            f.create_dataset('coeffs', data=cfs_all)
            f.create_dataset('frequencies', data=self._common_frequencies)
            if self.time_axis is not None:
                f.create_dataset('time_axis', data=self.time_axis)
            if self.coi is not None:
                f.create_dataset('coi', data=self.coi)

        print(f"CWT 结果已保存到 {output_path} (shape: {cfs_all.shape})")
