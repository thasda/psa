...
- 保存结果为 HDF5 文件，包含频率轴和 PSD 数据，同时复制输入文件的属性（通道名称等）
"""
...
class PSDCalculator:
    ...
    def __init__(self, hdf5_path: Union[str, Path], config_manager):
        ...
        self.input_path = self.hdf5_path   # 保存输入路径，用于复制属性
        self.cfg = config_manager.get_config_dict() if hasattr(config_manager, 'get_config_dict') else config_manager

        # 从配置中提取必要参数（支持多种键名）
        self.sample_rate = (
            self.cfg.get("采样率(Hz)") or
            self.cfg.get("采样率 (Hz)") or
            self.cfg.get("sample_rate")
        )
        if self.sample_rate is None:
            ...
            if self.sample_rate is None:
                raise ValueError("无法从配置或HDF5属性中获取采样率")

        self.nperseg = (
            self.cfg.get("FFT窗口长度") or
            self.cfg.get("fft_window_length")
        )
        if self.nperseg is None:
            with h5py.File(self.hdf5_path, 'r') as f:
                self.nperseg = f.attrs.get('nperseg', None)
            if self.nperseg is None:
                raise ValueError("无法从配置或HDF5属性中获取 FFT 窗口长度")
    ...
    def _save_results(self, output_path: Path, freqs: np.ndarray, psd: np.ndarray,
        ...
        """将 PSD 结果保存为 HDF5 文件，并复制输入文件的属性（如 channel_names）"""
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            # ---------- 复制输入 HDF5 的属性（如 channel_names） ----------
            if hasattr(self, 'input_path') and self.input_path:
                try:
                    with h5py.File(self.input_path, 'r') as fin:
                        for key, val in fin.attrs.items():
                            # 避免覆盖上面已设置的属性（如有冲突则以当前为准）
                            if key not in f.attrs:
                                f.attrs[key] = val
                    print("已复制输入文件的属性（包括通道名称）")
                except Exception as e:
                    print(f"复制输入属性时出错（不影响主结果）: {e}")
            # -------------------------------------------------
        print("保存完成")